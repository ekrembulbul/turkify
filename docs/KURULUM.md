...
7. [Sistem geneli kısayol (native uygulama)](#7-sistem-geneli-kısayol-native-uygulama)
...
| **Native uygulama** | Sistem geneli kısayol (seç → düzelt → yapıştır) | macOS uygulaması (bkz. [Bölüm 7](#7-sistem-geneli-kısayol-native-uygulama)) | CLI'den elle çalıştırırsın |

**Öneri:** Tier 1 + Tier 2 + native uygulama çoğu kullanıcı için en iyi denge
...
- **macOS** (öncelikli; çekirdek motor çok-platform — bkz. [Bölüm 8](#8-diğer-platformlar-windows--linux)).
...
- (Sistem geneli kısayol için) **macOS uygulaması** — bkz. [Bölüm 7](#7-sistem-geneli-kısayol-native-uygulama)
...
> yoksa Tier 3 atlanır. İlk çağrıda model belleğe yüklenir (yavaş); native
> uygulama/`serve` motoru sıcak tuttuğu için sonraki çağrılar hızlıdır.
...
için ~15–20 sn). Tier 3 zaten yalnızca gerçek belirsizliklerde çalıştığı ve
native uygulama/`serve` motoru sıcak tuttuğu için günlük kullanımda bu bekleme seyrektir.
...
**Ayar bayrakları** (düzeltme ve `serve` komutları kabul eder; hepsi config'i
geçersiz kılar):
...
## 7. Sistem geneli kısayol (native uygulama)

Herhangi bir uygulamada **seçili metni** kısayolla yerinde düzeltmek için
**native macOS uygulaması** kullanılır (menü-bar; kopyala → düzelt → yapıştır).
Uygulama motoru `turkify serve` ile arka planda sıcak tutar.

- Kurulum, çalıştırma ve kısayol kaydı: [`macos/README.md`](../macos/README.md).
- Varsayılan kısayollar: düzeltme **Hyper+A**, işlem iptali **Hyper+Q**;
  uygulama içinden "Değiştir" ile yeniden atanır (ayarlar native saklanır,
  config.json kullanılmaz — bkz. [ADR 0007](adr/0007-ayar-saklama-gui-native.md)).
- Tek gereken izin **Erişilebilirlik (Accessibility)**: pano/tuş simülasyonu
  (Cmd+C / Cmd+V) için zorunludur.

> Eski `turkify agent` (pynput tabanlı çok-platform ajan) **kaldırıldı**; sistem
> geneli kısayol artık native frontend'in görevidir ([ADR 0006](adr/0006-cli-birinci-sinif-kalici.md)).

---

## 8. Diğer platformlar (Windows / Linux)

Çekirdek motor ve CLI çok-platformdur; **öncelik macOS'tur**, diğerleri
...
**Sistem geneli kısayol tepki vermiyor (macOS)**
Native uygulamada **Erişilebilirlik (Accessibility) izni** verildi mi? (Sistem
Ayarları → Gizlilik ve Güvenlik → Erişilebilirlik). İzin ve kısayol kaydı için
...
```bash
# Native uygulama kuruluysa kapat (menü-bar → Çıkış)
