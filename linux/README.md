...
2. Eksik sistem araçlarını `apt` ile kurar: clipboard (`wl-clipboard`/`xclip`),
   `libnotify-bin`, `ydotool` *(sudo sorabilir)*.
3. **ydotool**: kullanıcıyı `input` grubuna ekler + servisini açar *(grup üyeliği için
   sonradan oturum yenilemek gerekir)*.
4. **Başlangıç dosyaları** (varsa **üzerine yazmaz**):
...
5. systemd `--user` unit'leri (sıcak motor + otomatik reload) yazar, enable+restart eder.
6. **GNOME kısayolunu** `gsettings` ile bağlar (varsa **dokunmaz**).

### Seçenekler

```bash
linux/install.sh --shortcut '<Control><Alt>y'   # kısayol kombinasyonu (varsayılan <Control><Super><Alt>a)
linux/install.sh --paste-delay-ms 400           # otomatik yapıştırma ıskalıyorsa gecikmeyi gömer
