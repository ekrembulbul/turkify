...
#      libnotify-bin                                                   [sudo]
#   3) config.json + protected_words.txt başlangıç dosyaları (varsa DOKUNMAZ)
#   4) systemd --user unit'leri (sıcak motor + otomatik reload) + enable/restart
#   5) GNOME kısayolunu gsettings ile bağla (varsa DOKUNMAZ)
#
# Yapıştırma MANUELDİR: düzeltilen metin panoya yazılır + bildirim çıkar, kullanıcı
# Ctrl+V ile yapıştırır (otomatik tuş enjeksiyonu/ydotool yoktur).
#
# Seçenekler:
...
PY
  ...
  gsettings set "$schema" command "$repo/linux/bin/turkify-fix"
...
if [[ ${#pkgs[@]} -gt 0 ]]; then
  ...
  log "sistem araçları hazır (clipboard, notify-send)"
fi

# --- 3) Başlangıç dosyaları (varsa ASLA üzerine yazma). Yollar turkify.config'ten ---
...
# --- 4) systemd --user unit'leri (motor servisi + otomatik reload) ---
...
==> Kurulum tamam. Kullanım: metni vurgula → kısayola bas → bildirim çıkar → Ctrl+V.
    Düzeltilmiş metin panoya yazılır; yapıştırmayı sen yaparsın (otomatik enjeksiyon yok).
